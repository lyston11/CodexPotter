...
/// (`supports-color` level, `WT_SESSION`, and `FORCE_COLOR`)
...
/// Map a raw [`StdoutColorLevel`] to a [`DiffColorLevel`] using Windows
/// Terminal-specific truecolor promotion rules.
///
...
/// is set. In codex-potter we promote truecolor when `WT_SESSION` is present
/// (unless `FORCE_COLOR` is set), matching upstream's default behavior for
...
mod tests {
    ...
    fn wt_session_promotes_ansi16_to_truecolor_for_diffs() {
        assert_eq!(
            diff_color_level_for_terminal(StdoutColorLevel::Ansi16, true, false),
            DiffColorLevel::TrueColor
        );
    }

    #[test]
    fn no_wt_session_keeps_ansi16_diff_palette() {
        assert_eq!(
            diff_color_level_for_terminal(StdoutColorLevel::Ansi16, false, false),
            DiffColorLevel::Ansi16
        );
    }
    ...
    fn wt_session_promotes_unknown_color_level_to_truecolor() {
        assert_eq!(
            diff_color_level_for_terminal(StdoutColorLevel::Unknown, true, false),
            DiffColorLevel::TrueColor
        );
    }

    #[test]
    fn no_wt_session_keeps_unknown_color_level_conservative() {
        assert_eq!(
            diff_color_level_for_terminal(StdoutColorLevel::Unknown, false, false),
            DiffColorLevel::Ansi16
        );
    }

    #[test]
    fn explicit_force_override_keeps_ansi16_on_wt_session() {
        assert_eq!(
            diff_color_level_for_terminal(StdoutColorLevel::Ansi16, true, true),
            DiffColorLevel::Ansi16
        );
    }

    #[test]
    fn explicit_force_override_keeps_ansi256_on_wt_session() {
        assert_eq!(
            diff_color_level_for_terminal(StdoutColorLevel::Ansi256, true, true),
            DiffColorLevel::Ansi256
        );
    }
