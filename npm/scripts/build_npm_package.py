#!/usr/bin/env python3
"""Stage and optionally package the codex-potter npm module."""
...
CODEX_POTTER_PLATFORM_PACKAGES: dict[str, dict[str, str]] = {
    "codex-potter-linux-x64": {
        "npm_name": "codex-potter-linux-x64",
        "npm_tag": "linux-x64",
        "target_triple": "x86_64-unknown-linux-musl",
        "os": "linux",
        "cpu": "x64",
    },
    "codex-potter-linux-arm64": {
        "npm_name": "codex-potter-linux-arm64",
        "npm_tag": "linux-arm64",
        "target_triple": "aarch64-unknown-linux-musl",
        "os": "linux",
        "cpu": "arm64",
    },
    "codex-potter-darwin-x64": {
        "npm_name": "codex-potter-darwin-x64",
        "npm_tag": "darwin-x64",
        "target_triple": "x86_64-apple-darwin",
        "os": "darwin",
        "cpu": "x64",
    },
    "codex-potter-darwin-arm64": {
        "npm_name": "codex-potter-darwin-arm64",
        "npm_tag": "darwin-arm64",
        "target_triple": "aarch64-apple-darwin",
        "os": "darwin",
        "cpu": "arm64",
    },
    "codex-potter-win32-x64": {
        "npm_name": "codex-potter-win32-x64",
        "npm_tag": "win32-x64",
        "target_triple": "x86_64-pc-windows-msvc",
        "os": "win32",
        "cpu": "x64",
    },
    "codex-potter-win32-arm64": {
        "npm_name": "codex-potter-win32-arm64",
        "npm_tag": "win32-arm64",
...
PACKAGE_EXPANSIONS: dict[str, list[str]] = {
    "codex-potter": ["codex-potter", *CODEX_POTTER_PLATFORM_PACKAGES],
}

PACKAGE_NATIVE_COMPONENTS: dict[str, list[str]] = {
    "codex-potter": [],
    "codex-potter-linux-x64": ["codex-potter"],
    "codex-potter-linux-arm64": ["codex-potter"],
    "codex-potter-darwin-x64": ["codex-potter"],
    "codex-potter-darwin-arm64": ["codex-potter"],
    "codex-potter-win32-x64": ["codex-potter"],
    "codex-potter-win32-arm64": ["codex-potter"],
}

PACKAGE_TARGET_FILTERS: dict[str, str] = {
    package_name: package_config["target_triple"]
    for package_name, package_config in CODEX_POTTER_PLATFORM_PACKAGES.items()
}

PACKAGE_CHOICES = tuple(PACKAGE_NATIVE_COMPONENTS)

COMPONENT_DEST_DIR: dict[str, str] = {
    "codex-potter": "codex-potter",
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build or stage the codex-potter npm package."
    )
    parser.add_argument(
        "--package",
        choices=PACKAGE_CHOICES,
        default="codex-potter",
        help="Which package to stage (default: codex-potter).",
    )
    parser.add_argument(
        "--version",
...
def prepare_staging_dir(staging_dir: Path | None) -> tuple[Path, bool]:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="codex-potter-npm-stage-"))
...
def stage_sources(staging_dir: Path, version: str, package: str) -> None:
    ...
    if package == "codex-potter":
        ...
        if not readme_src.exists():
            raise RuntimeError(f"Missing npm README: {readme_src}")
        shutil.copy2(readme_src, staging_dir / "README.md")

        package_json_path = NPM_ROOT / "package.json"
    elif package in CODEX_POTTER_PLATFORM_PACKAGES:
        platform_package = CODEX_POTTER_PLATFORM_PACKAGES[package]
        ...
        with open(NPM_ROOT / "package.json", "r", encoding="utf-8") as fh:
            codex_potter_package_json = json.load(fh)

        package_json = {
            "name": CODEX_POTTER_NPM_NAME,
            "version": platform_version,
            "license": codex_potter_package_json.get("license", "Apache-2.0"),
            "os": [platform_package["os"]],
            "cpu": [platform_package["cpu"]],
            "files": ["vendor"],
            "repository": codex_potter_package_json.get("repository"),
        }

        engines = codex_potter_package_json.get("engines")
        if isinstance(engines, dict):
            package_json["engines"] = engines

        package_manager = codex_potter_package_json.get("packageManager")
    ...
    if package == "codex-potter":
        ...
        package_json["optionalDependencies"] = {
            CODEX_POTTER_PLATFORM_PACKAGES[platform_package]["npm_name"]: (
                f"npm:{CODEX_POTTER_NPM_NAME}@"
                f"{compute_platform_package_version(version, CODEX_POTTER_PLATFORM_PACKAGES[platform_package]['npm_tag'])}"
            )
            for platform_package in PACKAGE_EXPANSIONS["codex-potter"]
            if platform_package != "codex-potter"
        }
...
def run_npm_pack(staging_dir: Path, output_path: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="codex-potter-npm-pack-") as pack_dir_str:
